...
export function UserDetailModal({
  ...
  // New tabs state
  const [activeTab, setActiveTab] = useState<'details' | 'clients' | 'announcers'>('details');
  ...
  useEffect(() => {
    if (activeTab === 'clients' || activeTab === 'announcers') {
      const fetchList = async () => {
        setLoadingList(true);
        try {
          const role = activeTab === 'clients' ? 'user' : 'announcer';
  ...
  const priceLabel = priceTypeMapping[pricing.type || 'hora'] || 'tarifa base';
      ...
      <div className="relative bg-[#fcfcfd] w-full max-w-7xl h-full md:h-[92vh] md:rounded-[2.5rem] shadow-[0_40px_80px_-15px_rgba(0,0,0,0.12)] overflow-hidden flex flex-col border border-white animate-in zoom-in slide-in-from-bottom-4 duration-500">
        ...
        <div className="px-6 md:px-10 py-5 bg-white border-b border-gray-100 shrink-0 z-20">
          <div className="flex flex-col gap-4">
            ...
            {/* TABS INDICATOR */}
            <div className="flex items-center gap-2 p-1.5 bg-gray-100/50 backdrop-blur-sm rounded-2xl w-fit border border-gray-200/50 ml-[-4px]">
              {[
                { id: 'details', label: 'Detalles Anuncio', icon: Sparkles },
                { id: 'clients', label: 'Clientes', icon: Users },
                { id: 'announcers', label: 'Anunciantes', icon: BadgeCheck }
              ].map((tab) => {
                const Icon = tab.icon;
                ...
                return (
                  <button
                    ...
                    className={cn(
                      "flex items-center gap-2 px-4 py-2 rounded-[1rem] text-[10px] font-black uppercase tracking-wider transition-all duration-300",
                      isActive
                        ? "bg-white text-gray-900 shadow-sm border border-gray-200"
                        : "text-gray-500 hover:text-gray-900 hover:bg-white/50"
                    )}
                  >
                    <Icon className={cn("w-3.5 h-3.5", isActive ? "text-rose-500" : "text-gray-400")} />
                    {tab.label}
                  </button>
                );
              })}
            ...
            <div className="animate-in fade-in slide-in-from-left-4 duration-500 space-y-8">
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white p-6 rounded-[2rem] border border-gray-100 shadow-sm">
                <div>
                  <h2 className="text-xl font-black text-gray-900 tracking-tight">
                    {activeTab === 'clients' ? 'Directorio de Clientes' : 'Directorio de Anunciantes'}
                  </h2>
                  <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mt-0.5">
                    {filteredList.length} perfiles encontrados
                  </p>
                </div>
                ...
                <div className="flex items-center gap-2">
                  <select
                    ...
                    className="bg-gray-50 border-none rounded-xl font-bold text-[10px] uppercase tracking-wider py-4 px-4 focus:ring-2 focus:ring-rose-500 shadow-inner"
                  >
              ...
              {loadingList ? (
                <div className="flex flex-col items-center justify-center py-20 gap-4">
                  ...
                  <p className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em]">Cargando perfiles...</p>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                  {filteredList.map((u) => (
                    <div
                      key={u.id}
                      className="bg-white rounded-[2rem] p-5 border border-gray-100 shadow-sm hover:shadow-xl hover:border-rose-200 transition-all group flex flex-col gap-4"
                    >
                      <div className="flex items-center gap-4">
                        <div className="w-16 h-16 rounded-2xl bg-gray-100 overflow-hidden border-2 border-white shadow-md shrink-0 group-hover:scale-105 transition-transform">
                          <img
                        ...
                        <div className="min-w-0">
                          <div className="flex items-center gap-1.5">
                            ...
                            {u.verified && <BadgeCheck className="w-3.5 h-3.5 text-blue-500" fill="currentColor" color="white" />}
                          </div>
                      ...
                      <div className="bg-gray-50 rounded-xl p-3 border border-gray-100/50">
                        <p className="text-[10px] text-gray-500 font-medium leading-relaxed line-clamp-2">
                          {u.bio || u.description || 'Sin descripción disponible.'}
                        </p>
                      </div>
                      ...
                      <div className="flex items-center justify-between pt-2">
                        <div className="flex items-center gap-1 text-amber-500">
                          ...
                          <span className="text-[10px] font-black text-gray-900">{u.rating || '5.0'}</span>
                        </div>
                        <button
                          onClick={() => onMessage?.(u.id || u.uid || '')}
                          className="px-4 py-2 bg-gray-900 text-white rounded-xl text-[9px] font-black uppercase tracking-widest hover:bg-rose-500 transition-colors shadow-lg active:scale-95"
                        >
                          Contactar
                        </button>
                      </div>
                ...
                <div className="text-center py-20 bg-gray-50 rounded-[3rem] border border-dashed border-gray-200">
                  ...
                  <h3 className="text-lg font-black text-gray-900">No se encontraron perfiles</h3>
                  <p className="text-xs font-bold text-gray-400">Prueba ajustando tu búsqueda.</p>
                </div>
              )}
            </div>
          )}
        ...
        {/* FLOATING ACTIONS */}
        {activeTab === 'details' && (
          <div className="absolute bottom-8 right-8 z-[70] flex flex-col items-end gap-5 group/actions animate-in fade-in slide-in-from-bottom-4 duration-500">
            {/* Favorite Bubble */}
            <button
